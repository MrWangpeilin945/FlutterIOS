...
const ExamHearing = ({
  ...
  useEffect(() => {
    ...
    validatedData = [左1000Hz, 左4000Hz, 右1000Hz, 右4000Hz].reduce(
      (item, detailNumber) => {
        ...
        //所見なしの場合は処理を行わない
        if (!detail.value || detail.value === getOptionCode(detail, 1))
          return item;
        //所見ありの場合は処理を行う
  ...
  const validationCheck = (item: InputExamItem) => {
    ...
    if (!isRegisterPressed) return { validateResult: item, hasCallback: true };
    // APIエラーメッセージで初期化
  ...
  const setFindings = (value: string, detailNumber: number) => {
    ...
    const { validateResult } = validationCheck(complementedItems);
    setExamItemData(validateResult);
