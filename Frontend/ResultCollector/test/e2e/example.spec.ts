...
import { BasePage } from "./POM/pages/base/BasePage";
...
test("シナリオ➀", async ({ page }) => {
	...
	for (let i = 0; i < testSetting.examMenuTestLoopCount; i++) {
		await basePage.goto("exammenu-select");
...
test("シナリオ➁", async ({ page }) => {
	...
	const targetPlaceSelectButton = new PlaceSelectPage(page).placeSelectButton;
	//班を選択
