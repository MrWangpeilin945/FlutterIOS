...
プロダクトコードとテストコードで大きく分割する。 +
メインはWellship.WebAPIに実装するが、APIモデルや例外は他のアプリ（CLIアプリ）から参照することもあるため、プロジェクトを分離して参照可能にする。 +
第二弾以降ではWELLSHIP共通機能となるものが出てくるが、初期開発時に分割するのは難しいため「結果収集（ResultCollector）」配下に配置する。
...
HTTPステータスコードを設定することで、Usecaseのメソッド内で例外送出した際、 +
クライアント側に設定したHTTPステータスコードが伝わる。
...
アプリからPostgreSQLへの接続が瞬断した場合でも自動復旧できるよう、データベース接続に関する処理にリトライ処理を実装する。 +
リトライ処理はPolly.Coreライブラリを使用する。

==== Resilience

``Ryobi.Wellship.WebAPI.ResultCollector.Infrastructure.Resilience``名前空間に``DbConnection``、``DbCommand``、``DbTransaction``を継承・ラップしたクラスを作成した。

これらのクラスは内部に実体のインスタンスを保持しており、定義されたメソッドを実体のインスタンスに中継する役割を持つ。リトライが必要な処理の場合はリトライ処理を挟んで中継する。
...
[source,json]
----
{ "date": "2025-02-04 16:44:10.8429", "level": "ERROR", "message": "○○処理に失敗しました。", "tenantKey": "stg01", "errorCode": "E002"}
----
...
アプリケーション内部のログレベルは `Microsoft.Extensions.Logging.LogLevel` を使用するが、 +
標準出力とカスタムログのログレベルは `NLog` のログレベル文字列として出力する。
以下に対応表を示す。
...
メッセージ以外に付加的な情報を書き込みたい場合は、details（ `<string, object>` 型の辞書）に値をセットする。 +
detailsがある場合は、app_logsテーブルのdetailsカラムにJSON文字列化して保存する。

WarnとErrorの場合は、エラーコードを指定して書き込む。エラーコードはEnumで管理する。 +
Enumを文字列に変換して記録する。

エラーコードはNLogのプロパティとapp_logsテーブルのdetailsカラムに記録する。 +
つまり任意のパラメータdetailsにerrorCodeを設定してはならない。 +
もし設定した場合は、メソッド引数のerrorCodeで上書きする。
...
class CustomLoggingService {
    ...
    +Task LogWarnAsync(string message, ErrorCode errorCode, Dictionary~string, object~? details = null)
    +Task LogErrorAsync(string message, ErrorCode errorCode, Dictionary~string, object~? details = null)
    -Task LogAsync(LogLevel level, ErrorCode errorCode, string message, Dictionary~string, object~? details)
}
...
* LogWarnAsync(message, errorCode, details)
* LogErrorAsync(message, errorCode, details)
...
sequenceDiagram
    ...
    rect rgb(216, 245, 255)
        ...
        CustomLoggingService->>NLog: LogInformation(message)
    end

    rect rgb(248, 226, 191)
        Client->>CustomLoggingService: LogWarnAsync(message, errorCode, details)
        CustomLoggingService->>LogRepository: WriteLogAsync(log)
        CustomLoggingService->>NLog: LogWarning(message)
    end

    rect rgb(253, 228, 232)
        Client->>CustomLoggingService: LogErrorAsync(message, errorCode, details)
        CustomLoggingService->>LogRepository: WriteLogAsync(log)
...
HTTPリクエストをトリガーにしてクライアント側がタイムアウトするレベルの巨大なデータを処理する場合、HTTPレスポンスを即時返却して別スレッドで処理を実行する。

Fire-And-Forgetパターンで処理をする場合、taskをawaitしないようにしただけではHTTPレスポンス返却後にDbConnectionなどがDisposeされることから、専用のTaskRunnerを用意した。
``Ryobi.Wellship.WebApi.Infrastructure.BackgroundTasks`` 名前空間の``IServiceScopeTaskRunner``の``Run``メソッドを使用することで、メソッドに渡した処理が非同期で実行される。

NOTE: このTaskRunnerに渡された処理はHttpContextなど元のスレッドの情報を引き継がないようになっています。``ITenantProvider``のみ情報を引き継ぐ処理をしていますが、他に引き継ぐ必要がある情報があればDIコンテナによってDisposeされないよう注意して引き渡してください。

CAUTION: 別スレッドで処理をするため、処理中にエラーが発生して処理が中断されてもその情報はクライアント側に伝わりません。使用する場合には運用フローの考慮と適切なロギングを実施してください。
...
表形式のデータを一足飛びにC#のコードにすると修正があった場合の手間が大きい。 +
そこでプレーンテキストでUMLのクラス図を出力させて中間生成物とするのがよい。
