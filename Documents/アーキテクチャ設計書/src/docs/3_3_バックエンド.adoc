...
アプリからPostgreSQLへの接続が瞬断した場合でも自動復旧できるよう、データベース接続に関する処理にリトライ処理を実装する。リトライ処理はPolly.Coreライブラリを使用する。
...
WarnとErrorの場合は、エラーコードを指定して書き込む。エラーコードはEnumで管理する。Enumを文字列に変換して記録する。

エラーコードはNLogのプロパティとapp_logsテーブルのdetailsカラムに記録する。つまり任意のパラメータdetailsにerrorCodeを設定してはならない。もし設定した場合は、メソッド引数のerrorCodeで上書きする。
