...
public class ExamineeUsecaseTests
{
    ...
    private readonly Mock<IAffiliationRepository> _affiliationRepositoryMock;
    ...
    public ExamineeUsecaseTests()
    {
        ...
        _affiliationRepositoryMock = new Mock<IAffiliationRepository>();
    ...
    public async Task 空値のチェック_受診者コードで1件のエラーが返る()
    {
        // Arrange
        // 団体コードを基に団体情報を取得する
        var existOrganizationCodes = Task.FromResult(new List<string> {"0001"});
        _organizationRepositoryMock.Setup(x => x.GetOrganizationsByCodesAsync(new List<string> {"0001"})).Returns(existOrganizationCodes);

        // WARNING検証エラー
        var expected = new List<ErrorObject>() {
            new(){Code = "10004", Message = "必須項目が不足しています。ExamineeCode", InputNote  = "UT2001"}
        };
        var usecase = new ExamineeUsecase(_organizationRepositoryMock.Object,  _examineeRepositoryMock.Object,
                                          _affiliationRepositoryMock.Object, _timeProvider);
    ...
    public async Task 空値のチェック_所属団体コードで2件のエラーが返る()
    {
        ...
        // 団体コードを基に団体情報を取得する
        var existOrganizationCodes = Task.FromResult(new List<string> {"0001"});
        _organizationRepositoryMock.Setup(x => x.GetOrganizationsByCodesAsync(new List<string> {""})).Returns(existOrganizationCodes);
    ...
    public async Task データのチェック_所属団体コードで1件のエラーが返る()
    {
        // Arrange
        // 団体コードを基に団体情報を取得する
        var existOrganizationCodes = Task.FromResult(new List<string> {""});
        _organizationRepositoryMock.Setup(x => x.GetOrganizationsByCodesAsync(new List<string> {"0001"})).Returns(existOrganizationCodes);
